...
//Code Here
let evenNumbers = mixedNumbers.filter(v => v%2 === 0); // = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let postTaxPrices = prices.map(v => v *= 1.07); // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((tot,val) => tot + val); //  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(v => v["CP"] > 200);// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map(v => v["price"] * (1 + v["tax"])); // Code here
...
let bobsTotal = purchases.filter(v => v['owner'] === 'Bob').map(v => v['price']).reduce((tot,val) => tot + val); //Code Here